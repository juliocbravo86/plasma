...
$art_style = '';
$art_head = '';
function sangre1_process_html(&$variables) {
...
function sangre1_breadcrumb($variables) {
 ...
 * @ingroup themeable
 */
function sangre1_button($variables) {
 ...
 */
function sangre1_preprocess_page(&$vars) {
  global $art_style, $art_head;
 ...
 * @ingroup themeable
 */
function sangre1_menu_local_task($variables) {
 ...
 */
function sangre1_feed_icon($variables) {
 ...
 * @ingroup themeable
 */
function sangre1_node_preview($variables) {
  $node = $variables['node'];
...
*/
function sangre1_form_alter(&$form, &$form_state, $form_id) {
...
function sangre1_form_element_label($variables) {
...
function sangre1_form_element($variables) {
  $element = &$variables['element'];
  // This is also used in the installer, pre-database setup.
  $t = get_t();
...
function sangre1_pager($variables) {
  $tags = $variables['tags'];
  $element = $variables['element'];
  ...
  if ($pager_total[$element] > 1) {
    ...
    return '<h2 class="element-invisible">' . t('Pages') . '</h2>' . art_item_list(array(
      'items' => $items,
...
function art_item_list($variables) {
  ...
  if (!empty($items)) {
    ...
    foreach ($items as $i => $item) {
      ...
      if (count($children) > 0) {
        // Render nested list.
        $data .= art_item_list(array('items' => $children, 'title' => NULL, 'type' => NULL, 'attributes' => $attributes));
      }
      if ($i == 0) {
